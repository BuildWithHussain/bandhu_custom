...
(function () {
	...
	// Server side, bandhu_app.bandhu_app.utils.realtime pushes this whenever a camp moves. It
	// carries only the camp name -- each board answers it by re-reading its own queues.
	...
	function add_refresh_icon(page, refresh) {
		...
		page.bandhu_refresh_icon = page.add_action_icon("refresh", () => refresh(), "", __("Refresh"));
	}
	...
	function format_detail_field(label, value, icon_name, fixed_width) {
		...
		return (
			...
			"</div>" +
			'<div class="bandhu-detail-value' + (fixed_width ? " bandhu-fixed-width" : "") + '">' +
