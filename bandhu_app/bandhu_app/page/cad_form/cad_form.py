...
def resolve_registration_origin(session: str) -> tuple[str | None, str | None]:
	...
	if not location:
		frappe.throw(
			_("This camp's site has no LSG set, so a Clinic ID cannot be issued. Ask an administrator to set it.")
		)
	if not frappe.db.get_value("Bandhu Location", location, "lsg_numeric_code"):
		frappe.throw(
			_("{0} has no LSG number yet, so a Clinic ID cannot be issued. Ask an administrator to set it.").format(location)
		)
	if not unit:
		frappe.throw(
	...
	if not frappe.db.get_value("Unit", unit, "unit_numeric_code"):
		frappe.throw(
			_("{0} has no unit number yet, so a Clinic ID cannot be issued. Ask an administrator to set it.").format(unit)
		)
...
def find_possible_duplicate(
	...
	candidates.append((_("the same name and date of birth"), {"patient_name": full_name, "dob": resolve_dob(dob, age)}))
