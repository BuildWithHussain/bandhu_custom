...
async function searchPatients(page) {
	...
	if (query.length < MIN_SEARCH_LENGTH) {
		frappe.msgprint(
			__("Type at least {0} characters to search.", [MIN_SEARCH_LENGTH])
		);
		return;
	}
...
function bindRegisterEvents(page) {
	page.main
		...
		.on("click", ".cad-register-toggle-btn", function () {
			...
			$(this).toggleClass("btn-primary", !form.is(":visible")).toggleClass("btn-default", form.is(":visible"));
		});
...
function confirmRegisterAnyway(page, existing) {
	return new Promise((resolve) => {
		const label = [existing.patient_name, bandhu.session_ui.group_clinic_id(existing.clinic_id), existing.age]
			.filter(Boolean)
			.join(" · ");
			...
			fields: [
				{
					...
					options:
						...
						"</p><p>" +
						__("Registering again gives this person a second Clinic ID and a second card.") +
						"</p>",
				},
			],
...
// The front desk is the one asked "how much longer", so the queue says how long each patient
// has been on it. Finished visits drop it -- a completed row's age is not a wait.
function format_waited(row) {
	...
	return (
		'<span class="queue-waited">' +
		__("waiting {0}", [frappe.datetime.comment_when(row.queued_at, true)]) +
		"</span>"
	);
}
...
frappe.pages["cad-form"].on_page_load = function (wrapper) {
	...
	page.set_secondary_action(__("My Schedule"), () => frappe.set_route("my-schedule"), "calendar");
