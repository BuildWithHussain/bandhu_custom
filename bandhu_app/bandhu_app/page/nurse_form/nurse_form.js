...
function renderQueueOrder(encounter, action) {
	...
	return (
		'<span class="queue-order">' + frappe.utils.escape_html(named.join(", ")) + "</span>"
	);
}
...
function renderTimeInCamp(encounter) {
	...
	return (
		'<span class="queue-in-camp">' +
		__("in camp {0}", [frappe.datetime.comment_when(encounter.creation, true)]) +
		"</span>"
	);
}
	...
	const rows = encounters
		.map(
			(encounter) =>
				...
				"</td>" +
				(action ? '<td class="waited-cell">' + renderTimeInCamp(encounter) + "</td>" : "") +
